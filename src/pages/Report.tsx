...
const Report = function () {
    ...
    const [reportCategory, setReportCategory] =
        useState<CourseReportCategory>("SuperGeeks");
    ...
    const InputChanger = function <T>(
        event: React.ChangeEvent<T & { value: string }>,
        name: string,
    ) {
    ...
    return (
        <>
            ...
            <Select
                name="course"
                value={report.course}
                onChange={(event) => InputChanger(event, "course")}
            >
                {Object.keys(Courses).map(function (optionGroupLabel) {
                    const keyOptionGroupElement = uuidv4();
                    return (
                        <optgroup
                            key={keyOptionGroupElement}
                            label={optionGroupLabel}
                        >
                            {Courses[
                                optionGroupLabel as keyof typeof Courses
            ...
            <Select
                name="class"
                value={report.class}
                onChange={(event) => InputChanger(event, "class")}
            >
            ...
            <Select
                ...
                onChange={(event) => InputChanger(event, "weekday")}
            >
